...
const RatedPage = ({ onLogout }) => {
    ...
    const [searchValue, setSearchValue] = useState(null);  // New state for AutoComplete
    ...
    const autocomplete = (e) => {
        ...
        setSearchValue(selected);  // Update searchValue instead of selectedContent
    ...
    const menuEnd = (
        <div className="flex align-items-center gap-2 items-center">
            <AutoComplete
                placeholder='Buscar'
                field="title"
                value={searchValue}  // Changed from selectedContent to searchValue
                suggestions={filteredContent}
                completeMethod={search}
                onChange={autocomplete}
            />
            <Avatar image={logoutIcon} shape="circle" onClick={onLogout} />
        </div>
    );
